...
def data_generator(batch_size = 1,dataset = 'train'):
    image_list = os.listdir('resized_data/' + dataset + '/')
    np.random.shuffle(image_list)
    min_mask = np.ones((16,1280,1918))
    max_mask = np.zeros((16,1280,1918))
    for angle_id in range(0,16):
        min_mask[angle_id] = misc.imread('min_and_max_masks_1280/' + 'min_' + str(angle_id) + '.jpg',flatten=True)/255.
        max_mask[angle_id] = misc.imread('min_and_max_masks_1280/' + 'max_' + str(angle_id) + '.jpg',flatten=True)/255.
    while 1:
        for batch_num in range(len(image_list)//batch_size):
            ...
            X = np.zeros((batch_size, 1280, 1920, 12), dtype=np.float128)
            ...
            for i,image_name in zip(range(batch_size),batch_images):
                ...
                image_bw = misc.imread('train/' + image_name,flatten=True)/255.
                image_all_mask = misc.imread('train_all_masks/' + image_name.split('_')[0] + '.jpg', flatten=True)/255.
                image_mask = misc.imread('train_masks/' + image_name.split('.')[0] + '_mask.gif', flatten = True)/255.
#                image_mask = np.reshape(image_mask,(1280, 1918, 1))
                X[i,...,1:1919,:3] = image_rgb
                X[i,...,1:1919,3] = image_all_mask
                X[i,...,1:1919,4] = min_mask[int(image_name.split('.')[0].split('_')[1]) - 1]
                X[i,...,1:1919,5] = max_mask[int(image_name.split('.')[0].split('_')[1]) - 1]
                ...
                X[i,...,9] = df.l2_dist.reshape((1280, 1920))
                X[i,...,10] = df.l1_dist_x.reshape((1280, 1920))
                X[i,...,11] = df.l1_dist_y.reshape((1280, 1920))
                Y[i,...,1:1919,0] = image_mask
                i = i + 1
            yield X, Y

train_data_gen = data_generator(1)
val_data_gen = data_generator(1,'val')

#Create simple model
inp = Input((1280,1920,12))
...
conv4 = Conv2D(12, 3, activation='relu', padding='same')(max3)
max4 = MaxPooling2D(2)(conv4)
conv5 = Conv2D(12, 3, activation='relu', padding='same')(max4)
max5 = MaxPooling2D(2)(conv5)
conv6 = Conv2D(12, 3, activation='relu', padding='same')(max5)

deconv6 = Conv2DTranspose(6, 3, strides=32, activation='relu', padding='same')(conv6)
deconv5 = Conv2DTranspose(6, 3, strides=16, activation='relu', padding='same')(conv5)
deconv4 = Conv2DTranspose(6, 3, strides=8, activation='relu', padding='same')(conv4)
deconv3 = Conv2DTranspose(6, 3, strides=4, activation='relu', padding='same')(conv3)
deconv2 = Conv2DTranspose(6, 3, strides=2, activation='relu', padding='same')(conv2)

deconvs = concatenate([conv1, deconv2, deconv3, deconv4, deconv5, deconv6])
...
history = model.fit_generator(train_data_gen, epochs = 5, steps_per_epoch= 4096,
                                validation_data = val_data_gen, validation_steps = 512,verbose = 1)

model.save('keras_unet_1280_1920.hd5')
print 'Model Saved!'
model = load_model('keras_unet_1280_1920.hd5',custom_objects={'dice_loss':dice_loss,'dice_coef':dice_coef})
