...
train_data_gen = data_generator(4,'train')
val_data_gen = data_generator(4,'val')

#Create simple model
inp = Input((640,960,12))

conv11 = Conv2D(12, 3, activation='relu', padding='same')(inp)
conv12 = Conv2D(12, 3, activation='relu', padding='same')(conv11)
max1 = MaxPooling2D(2)(conv12)
conv21 = Conv2D(12, 3, activation='relu', padding='same')(max1)
conv22 = Conv2D(12, 3, activation='relu', padding='same')(conv21)
max2 = MaxPooling2D(2)(conv22)
conv31 = Conv2D(12, 3, activation='relu', padding='same')(max2)
conv32 = Conv2D(12, 3, activation='relu', padding='same')(conv31)
max3 = MaxPooling2D(2)(conv32)
conv41 = Conv2D(12, 3, activation='relu', padding='same')(max3)
conv42 = Conv2D(12, 3, activation='relu', padding='same')(conv41)
max4 = MaxPooling2D(2)(conv42)
conv5 = Conv2D(12, 3, activation='relu', padding='same')(max4)

deconv5 = Conv2DTranspose(12, 3, strides=16, activation='relu', padding='same')(conv5)
deconv4 = Conv2DTranspose(12, 3, strides=8, activation='relu', padding='same')(conv42)
deconv3 = Conv2DTranspose(12, 3, strides=4, activation='relu', padding='same')(conv32)
deconv2 = Conv2DTranspose(12, 3, strides=2, activation='relu', padding='same')(conv22)

deconvs = concatenate([conv12, deconv2, deconv3, deconv4, deconv5])
...
history = model.fit_generator(train_data_gen, epochs = 30, steps_per_epoch= 1024,initial_epoch=20,
                                validation_data = val_data_gen, validation_steps = 128,verbose = 1)

model.save('keras_unet_640_960.hd5')

model = load_model('keras_unet_640_960.hd5',custom_objects={'dice_loss':dice_loss,'dice_coef':dice_coef})
...
def show_pred_mask(num):
    ...
    act_mask = misc.imread('train_masks_resized_640/' + image_list[num].split('.')[0] + '_mask.jpg',flatten=True)/255.
